...
public class Map : MonoBehaviour {
	...
	public List<Pair<int,int>> findAdjacentTile(int currentx, int currenty){
		int max_x = tiles.GetLength (0);
		int max_y = tiles.GetLength (1);
		
		// hand made adjacent tiles
		Pair<int, int> North = new Pair<int, int>(currentx, currenty+1);
		Pair<int, int> South = new Pair<int, int>(currentx, currenty-1);
		Pair<int, int> East = new Pair<int, int>(currentx+1, currenty);
		Pair<int, int> West = new Pair<int, int>(currentx-1, currenty);

		List< Pair<int, int> > adjacent = new List<Pair<int, int>>{North, South, East, West};

		List< Pair<int, int>> allowedAdjacent = new List<Pair<int, int>>{};

		foreach( Pair<int, int> i in adjacent){
			if (i.First <= max_x && i.First >= 0 && i.Second <= max_y && i.Second >= 0){
				allowedAdjacent.Add(i);
			}
		}

		return allowedAdjacent;
	}
